...
let inline isNull<'a when 'a : not struct> (x : 'a) = referenceEquals (box x) null
...
let inline concat4 (a : string) (b : string) (c : string) (d : string) =
    System.String.Concat (a, b, c, d)

let inline concat5 (a : string) (b : string) (c : string) (d : string) (e : string) =
    System.String.Concat ([| a ; b ; c ; d ; e |])

let inline concat6 (a : string) (b : string) (c : string) (d : string) (e : string) (f : string) =
    System.String.Concat ([| a ; b ; c ; d ; e ; f |])
...
let inline concat7
    ...
    (g : string)
    =
    System.String.Concat ([| a ; b ; c ; d ; e ; f ; g |])
...
type LineWrapper
    (
        ...
        columnWidth : int,
        writerIsMultiCharGraphemeSafe : bool
    ) =
    do
        if columnWidth < 1 then
            invalidArg "columnWidth" "columnWidth must be positive."
    ...
    member t.Indentation
        ...
        and set (s : string) =
            let s =
                if s.Length <= columnWidth - 1 then
                    s
                else
                    s.Substring (0, columnWidth - 1) // guarantee maxSpace >= 1
    ...
    member t.Space () = afterSpace <- true
        ...
        if isNotNull s then
            ...
            if start < s.Length then
                if start = 0 then
                    ...
                    t.Write (s.Substring (start, s.Length - start))

    member t.Print (s1, s2) =
        t.Print (s1)
        t.Print (s2)

    member t.Print (s1, s2, s3) =
        t.Print (s1)
        t.Print (s2)
        t.Print (s3)

    member t.PrintLine (s : string) =
        t.Print (s)
        t.Newline ()

    member t.PrintLine (s1 : string, s2 : string) =
        t.Print (s1)
        t.Print (s2)
        t.Newline ()

    member t.PrintLine (s1 : string, s2 : string, s3 : string) =
        t.Print (s1)
        t.Print (s2)
        t.Print (s3)
        t.Newline ()
...
let getLineSnippet
    ...
    if colTooLarge then
        let mutable n =
            if p.Column > int64 System.Int32.MaxValue then
                maxExtraChars
            else
    ...
    if nTabs > 0 then // replace tabs with spaces
        let mutable off =
            if columnOffset = 0L then
                0
            else
                int32 (columnOffset % (int64 tabSize))
        ...
        if i0 < str.Length then
            sb.Append (str, i0, str.Length - i0) |> ignore
    ...
    if not mayContainMultiCharGraphemes then
        let nBefore, nAfter =
            clip idx (if idx < str.Length then str.Length - idx - 1 else 0)

        {
            String =
                str.Substring (
                    idx - nBefore,
                    nBefore + nAfter + (if idx < str.Length then 1 else 0)
                )
            Index = nBefore
        ...
        while idxIdx < indices.Length && indices.[idxIdx] < idx do
            idxIdx <- idxIdx + 1

        if
            (if idxIdx < indices.Length then
                 indices.[idxIdx] > idx
             else
                 idx < str.Length)
        then
        ...
        let teLength =
            (if idxIdx + 1 < indices.Length then
                 indices.[idxIdx + 1]
             else
                 str.Length)
            - teIdx

        let mutable nBefore, nAfter =
            clip
                idxIdx
                (if idxIdx = indices.Length then
                     0
                 else
                     indices.Length - idxIdx - 1)

        let mutable strBegin =
            let ii = idxIdx - nBefore in if ii < indices.Length then indices.[ii] else str.Length

        let mutable strEnd =
            let ii = idxIdx + nAfter + 1 in if ii < indices.Length then indices.[ii] else str.Length
