...
let (|Expected|_|) (msg : ErrorMessage) =
    if msg.Type = ErrorMessageType.Expected then
        Some msg.String
    else
        None

let (|ExpectedString|_|) (msg : ErrorMessage) =
    if msg.Type = ErrorMessageType.ExpectedString then
        Some msg.String
    else
        None

let (|ExpectedStringCI|_|) (msg : ErrorMessage) =
    if msg.Type = ErrorMessageType.ExpectedCaseInsensitiveString then
        Some msg.String
    else
        None

let (|Unexpected|_|) (msg : ErrorMessage) =
    if msg.Type = ErrorMessageType.Unexpected then
        Some msg.String
    else
        None

let (|UnexpectedString|_|) (msg : ErrorMessage) =
    if msg.Type = ErrorMessageType.UnexpectedString then
        Some msg.String
    else
        None

let (|UnexpectedStringCI|_|) (msg : ErrorMessage) =
    if msg.Type = ErrorMessageType.UnexpectedCaseInsensitiveString then
        Some msg.String
    else
        None

let (|Message|_|) (msg : ErrorMessage) =
    if msg.Type = ErrorMessageType.Message then
        Some msg.String
    else
        None
...
let (|CompoundError|_|) (msg : ErrorMessage) =
    if msg.Type = ErrorMessageType.CompoundError then
        ...
        Some (
            (ce.LabelOfCompound,
             ce.NestedErrorPosition,
             ce.NestedErrorUserState,
             ce.NestedErrorMessages)
        )
    else
        None
...
let expected label =
    ErrorMessageList (ErrorMessage.Expected (label))

let expectedString str =
    ErrorMessageList (ErrorMessage.ExpectedString (str))

let expectedStringCI str =
    ErrorMessageList (ErrorMessage.ExpectedCaseInsensitiveString (str))

let unexpected label =
    ErrorMessageList (ErrorMessage.Unexpected (label))

let unexpectedString str =
    ErrorMessageList (ErrorMessage.UnexpectedString (str))

let unexpectedStringCI str =
    ErrorMessageList (ErrorMessage.UnexpectedCaseInsensitiveString (str))

let messageError msg =
    ErrorMessageList (ErrorMessage.Message (msg))

let otherError obj =
    ErrorMessageList (ErrorMessage.Other (obj : obj))
...
let nestedError (stream : CharStream<'u>) (error : ErrorMessageList) =
    ...
    *)
    if error |> isSingleErrorMessageOfType ErrorMessageType.NestedError then
        error
    else
        ErrorMessageList (ErrorMessage.NestedError (stream.Position, stream.UserState, error))
...
let compoundError label (stream : CharStream<'u>) (error : ErrorMessageList) =
    ...
    *)
    if error |> isSingleErrorMessageOfType ErrorMessageType.NestedError then
        ...
        ErrorMessageList (
            ErrorMessage.CompoundError (label, ne.Position, ne.UserState, ne.Messages)
        )
    else
        ErrorMessageList (
            ErrorMessage.CompoundError (label, stream.Position, stream.UserState, error)
        )

let inline mergeErrors errorMessages1 errorMessages2 =
    ErrorMessageList.Merge (errorMessages1, errorMessages2)

/// the default position printer
let internal printPosition
...
let internal printErrorPosition
    (tabSize : int)
    (lw : LineWrapper)
    (stream : CharStream<'u>)
    (p : Position)
    ...
    let writeStringWithSimplifiedWhitespace (tw : TextWriter) (s : string) =
        ...
        if i0 < s.Length then
            if i0 = 0 then
                ...
                tw.Write (s.Substring (i0, s.Length - i0))

    let sn =
        getLineSnippet
            stream
            p
            (lw.ColumnWidth - lw.Indentation.Length)
            tabSize
            lw.WriterIsMultiCharGraphemeSafe
        ...
        if System.Char.IsSurrogate (c) then
            if Text.IsHighSurrogate (c) then
                if i + 1 < str.Length && Text.IsLowSurrogate (str.[i + 1]) then
                    msgs.Add (
                        Strings.ErrorOccurredAtBeginningOfSurrogatePair (str.Substring (i, 2))
                    )
                else
                    msgs.Add (Strings.CharAtErrorPositionIsIsolatedHighSurrogate (c))
            else // low surrogate
            if
                i > 0 && Text.IsHighSurrogate (str.[i - 1])
            then
                msgs.Add (
                    Strings.ErrorOccurredAtSecondCharInSurrogatePair (str.Substring (i - 1, 2))
                )
            else
    ...
    else if p.Index = stream.IndexOfLastCharPlus1 then
        msgs.Add (Strings.ErrorOccurredAtEndOfInputStream)
    elif str.Length = 0 then
        msgs.Add (Strings.ErrorOccurredOnAnEmptyLine)
    else
        msgs.Add (Strings.ErrorOccurredAtEndOfLine)

    if
        sn.LengthOfTextElement > 1
        && (sn.LengthOfTextElement > 2 || not (System.Char.IsSurrogate (str.[sn.Index])))
    then
    ...
    if msgs.Count = 1 then
        lw.PrintLine (Strings.Note, msgs.[0])
...
type ParserError (position : Position, userState : obj, messages : ErrorMessageList) =
    ...
    member private t.WriteTo
        ...
        let rec printMessages (position : Position) (msgs : ErrorMessageList) =
            ...
            for msg in ErrorMessageList.ToSortedArray (msgs) do
                ...
                | ErrorMessageType.ExpectedCaseInsensitiveString ->
                    expectedA.Add (Strings.QuoteCaseInsensitive (msg.String))
                ...
                | ErrorMessageType.UnexpectedCaseInsensitiveString ->
                    unexpectedA.Add (Strings.QuoteCaseInsensitive (msg.String))
                ...
                | ErrorMessageType.CompoundError ->
                    if not (isNullOrEmpty msg.String) then
                        expectedA.Add (msg.String)

                    let ce = msg :?> ErrorMessage.CompoundError
                    compoundA.Add ((ce.String, ce.NestedErrorPosition, ce.NestedErrorMessages))
                | ErrorMessageType.Other -> otherCount <- otherCount + 1
    ...
    override t.GetHashCode () =
        t.Position.GetHashCode () ^^^ hash t.Messages
