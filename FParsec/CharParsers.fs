...
let foldCase : string -> string = Text.FoldCase
...
type ParserResult<'Result, 'UserState> =
    ...
    member private t.StructuredFormatDisplay =
        match t with
        | Success (r, _, _) ->
            if typeof<'Result> = typeof<unit> then
                "Success: ()"
            else
                sprintf "Success: %A" r
        | Failure (msg, _, _) -> sprintf "Failure:\n%s" msg
...
let runParserOnString
    (parser : Parser<'Result, 'UserState>)
    (ustate : 'UserState)
    (streamName : string)
    (chars : string)
    =
...
let runParserOnStream
    (parser : Parser<'Result, 'UserState>)
    (ustate : 'UserState)
    (streamName : string)
    (byteStream : System.IO.Stream)
    (encoding : System.Text.Encoding)
    =
#if LOW_TRUST
#else
#endif
    use stream = new CharStream<'UserState> (byteStream, encoding)
...
#else
#endif
    use stream = new CharStream<'UserState> (path, encoding)
    stream.UserState <- ustate
    applyParser parser stream
#endif

let run parser (string : string) = runParserOnString parser () "" string
...
let getPosition : Parser<Position, 'u> = fun stream -> Reply (stream.Position)

let getUserState : Parser<'u, 'u> = fun stream -> Reply (stream.UserState)
...
let newlineReturn result : Parser<_, 'u> =
    fun stream ->
        if stream.SkipNewline () then
            Reply (result)
        else
            Reply (Error, Errors.ExpectedNewline)

let newline<'u> : Parser<_, 'u> = newlineReturn '\n'
let skipNewline<'u> : Parser<_, 'u> = newlineReturn ()

let unicodeNewlineReturn result : Parser<_, 'u> =
    fun stream ->
        if stream.SkipUnicodeNewline () then
            Reply (result)
        else
            Reply (Error, Errors.ExpectedNewline)

let unicodeNewline<'u> : Parser<_, 'u> = unicodeNewlineReturn '\n'
let skipUnicodeNewline<'u> : Parser<_, 'u> = unicodeNewlineReturn ()

let internal charReturnE (c : char) result error : Parser<'a, 'u> =
    fun stream ->
        if stream.Skip (c) then
            Reply (result)
        else
            Reply (Error, error)
...
let charReturn c result : Parser<'a, 'u> =
    ...
    | EOS ->
        invalidArg
            "c"
            "The char '\uffff' (EOS) is not a valid argument for the pchar/skipChar/charReturn parser. If you want to check for the end of the stream, consider using the `eof` parser."
...
let skipAnyChar : Parser<unit, 'u> =
    fun stream ->
        if stream.ReadCharOrNewline () <> EOS then
            Reply (())
        else
            Reply (Error, Errors.ExpectedAnyChar)
...
let asciiUpper stream =
    fastInlineSatisfyE isAsciiUpper Errors.ExpectedAsciiUppercaseLetter stream

let asciiLower stream =
    fastInlineSatisfyE isAsciiLower Errors.ExpectedAsciiLowercaseLetter stream

let asciiLetter stream =
    fastInlineSatisfyE isAsciiLetter Errors.ExpectedAsciiLetter stream

// unicode is the default for letters and ascii the default for numbers
let upper stream =
    fastInlineSatisfyE isUpper Errors.ExpectedUppercaseLetter stream

let lower stream =
    fastInlineSatisfyE isLower Errors.ExpectedLowercaseLetter stream

let letter stream =
    fastInlineSatisfyE isLetter Errors.ExpectedLetter stream

let digit stream =
    fastInlineSatisfyE isDigit Errors.ExpectedDecimalDigit stream

let hex stream =
    fastInlineSatisfyE isHex Errors.ExpectedHexadecimalDigit stream

let octal stream =
    fastInlineSatisfyE isOctal Errors.ExpectedOctalDigit stream

let tab stream =
    fastInlineSatisfyE ((=) '\t') Errors.ExpectedTab stream
...
let spaces1 : Parser<unit, 'u> =
    fun stream ->
        if stream.SkipWhitespace () then
            Reply (())
        else
            Reply (Error, Errors.ExpectedWhitespace)
...
let unicodeSpaces1 : Parser<unit, 'u> =
    fun stream ->
        if stream.SkipUnicodeWhitespace () then
            Reply (())
        else
            Reply (Error, Errors.ExpectedWhitespace)
...
let eof : Parser<unit, 'u> =
    fun stream ->
        if stream.IsEndOfStream then
            ...
            Reply (Error, Errors.ExpectedEndOfInput)
...
let internal checkStringContainsNoNewlineOrEOSChar s name =
    ...
    if i >= 0 then
        newlineOrEOSCharInStringArg name s i
...
let stringCIReturn (s : string) result : Parser<'a, 'u> =
    ...
    if s.Length = 1 then
        ...
        if not (isCertainlyNoNLOrEOS c) then
            ...
            | '\r'
            | '\n'
            | EOS -> newlineOrEOSCharInStringArg "skipStringCI/stringCIReturn" s 0
            | _ -> ()

        let cfc = Text.FoldCase (c)

        fun stream ->
            if stream.SkipCaseFolded (cfc) then
                Reply (result)
            else
                Reply (Error, error)
    else
...
let skipAnyString n : Parser<unit, 'u> =
    ...
    fun stream ->
        ...
        if stream.SkipCharsOrNewlines (n) = n then
            Reply (())
        else
            stream.BacktrackTo (state)
            Reply (Error, error)
...
let restOfLine skipNewline : Parser<_, _> =
    fun stream -> Reply (stream.ReadRestOfLine (skipNewline))
...
let charsTillString (s : string) skipString maxCount : Parser<string, 'u> =
    ...
    if maxCount < 0 then
        raise (System.ArgumentOutOfRangeException ("maxCount", "maxCount is negative."))
    ...
    fun stream ->
        ...
        stream.SkipCharsOrNewlinesUntilString (s, maxCount, true, &charsBeforeString)
        |> ignore
...
let charsTillStringCI (s : string) skipString maxCount : Parser<string, 'u> =
    ...
    fun stream ->
        ...
        stream.SkipCharsOrNewlinesUntilCaseFoldedString (cfs, maxCount, true, &charsBeforeString)
        |> ignore
...
let skipCharsTillStringCI (s : string) skipString maxCount : Parser<unit, 'u> =
    ...
    fun stream ->
        ...
        stream.SkipCharsOrNewlinesUntilCaseFoldedString (cfs, maxCount, &foundString)
        |> ignore
...
let inline internal manySatisfyImpl
    require1
    (f1 : char -> bool)
    (f : char -> bool)
    error
    ...
    fun stream ->
        ...
        if not require1 || str.Length <> 0 then
            Reply (str)
        else
            Reply (Error, error)

let inline internal skipManySatisfyImpl
    require1
    (f1 : char -> bool)
    (f : char -> bool)
    error
    ...
    fun stream ->
        ...
        if not require1 || n <> 0 then
            Reply (())
        else
            Reply (Error, error)

let manySatisfy2 f1 f =
    manySatisfyImpl false f1 f NoErrorMessages

let many1Satisfy2 f1 f =
    manySatisfyImpl true f1 f NoErrorMessages

let many1Satisfy2L f1 f label =
    manySatisfyImpl true f1 f (expected label)

let skipManySatisfy2 f1 f =
    skipManySatisfyImpl false f1 f NoErrorMessages

let skipMany1Satisfy2 f1 f =
    skipManySatisfyImpl true f1 f NoErrorMessages

let skipMany1Satisfy2L f1 f label =
    skipManySatisfyImpl true f1 f (expected label)
...
let internal manyMinMaxSatisfy2E minCount maxCount f1 f error : Parser<string, 'u> =
    ...
    if minCount > 0 then
        ...
        fun stream -> Reply (stream.ReadCharsOrNewlinesWhile (f1, f, 0, maxCount, true))
...
let internal skipManyMinMaxSatisfy2E minCount maxCount f1 f error : Parser<unit, 'u> =
    ...
    if minCount > 0 then
        fun stream ->
            ...
            if n <> 0 then Reply (()) else Reply (Error, error)
    else
        fun stream ->
...
let manyMinMaxSatisfy minCount maxCount f =
    manyMinMaxSatisfy2E minCount maxCount f f NoErrorMessages

let manyMinMaxSatisfyL minCount maxCount f label =
    manyMinMaxSatisfy2E minCount maxCount f f (expected label)

let manyMinMaxSatisfy2 minCount maxCount f1 f =
    manyMinMaxSatisfy2E minCount maxCount f1 f NoErrorMessages

let manyMinMaxSatisfy2L minCount maxCount f1 f label =
    manyMinMaxSatisfy2E minCount maxCount f1 f (expected label)

let skipManyMinMaxSatisfy minCount maxCount f =
    skipManyMinMaxSatisfy2E minCount maxCount f f NoErrorMessages

let skipManyMinMaxSatisfyL minCount maxCount f label =
    skipManyMinMaxSatisfy2E minCount maxCount f f (expected label)

let skipManyMinMaxSatisfy2 minCount maxCount f1 f =
    skipManyMinMaxSatisfy2E minCount maxCount f1 f NoErrorMessages

let skipManyMinMaxSatisfy2L minCount maxCount f1 f label =
    skipManyMinMaxSatisfy2E minCount maxCount f1 f (expected label)


let internal regexE pattern error : Parser<string, 'u> =
    let regex =
        new Regex ("\\A" + pattern, RegexOptions.Multiline ||| RegexOptions.ExplicitCapture)
    ...
    fun stream ->
        ...
        if m.Success then
            ...
            if findNewlineOrEOSChar str < 0 then
                ...
                if n = nStr.Length then
                    Reply (nStr)
                else
                    Reply (
                        FatalError,
                        messageError
                            "Internal error in the regex parser. Please report this error to fparsec@quanttec.com."
                    )
        else
            Reply (Error, error)

let regex pattern =
    regexE pattern (Errors.ExpectedStringMatchingRegex (pattern))
...
type IdentifierOptions
 ...
 =
    ...
    let isAsciiIdStart =
        match isAsciiIdStart with
        | Some v -> v
        | _ -> IdentifierValidator.IsXIdStartOrSurrogate

    let isAsciiIdContinue =
        match isAsciiIdContinue with
        | Some v -> v
        | _ -> IdentifierValidator.IsXIdContinueOrSurrogate
#if PCL
#else
    ...
    let invalidCharError =
        messageError (
            defaultArg
                invalidCharMessage
                Strings.IdentifierContainsInvalidCharacterAtIndicatedPosition
        )

    let allowAllNonAsciiCharsInPreCheck =
        defaultArg allowAllNonAsciiCharsInPreCheck false

    let preCheckStart =
        if preCheckStart.IsSome then preCheckStart.Value
        elif allowAllNonAsciiCharsInPreCheck then isAsciiIdStart
        else Unchecked.defaultof<_>

    let preCheckContinue =
        if preCheckContinue.IsSome then preCheckContinue.Value
    ...
    do
        for i = 1 to 127 do
            ...
            if isAsciiIdStart c then
                v <- v ||| IdFlags.NonContinue

            if isAsciiIdContinue c then
                v <- v ||| IdFlags.Continue

            if allowAllNonAsciiCharsInPreCheck then
                if preCheckStart c then
                    v <- v ||| IdFlags.PreCheckNonContinue

                if preCheckContinue c then
                    v <- v ||| IdFlags.PreCheckContinue
        ...
        if allowAllNonAsciiCharsInPreCheck then
            fun c ->
                ...
                if i <= 0x7f then
                    ...
                    true
        elif isNotNull preCheckStart then
            preCheckStart
        else
            iv.IsIdStartOrSurrogateFunc
        ...
        if allowAllNonAsciiCharsInPreCheck then
            fun c ->
                ...
                if i <= 0x7f then
                    ...
                    true
        elif isNotNull preCheckContinue then
            preCheckContinue
        else
            iv.IsIdContinueOrJoinControlOrSurrogateFunc
...
let manyCharsTillApply2 p1 p endp f =
    ManyCharsTill(p1, p, endp, f).AsFSharpFunc

let manyCharsTillApply p endp f = manyCharsTillApply2 p p endp f

let manyCharsTill2 p1 p endp =
    manyCharsTillApply2 p1 p endp (fun str _ -> str)

let manyCharsTill p endp = manyCharsTill2 p p endp

let many1CharsTillApply2 p1 p endp f =
    Many1CharsTill(p1, p, endp, f).AsFSharpFunc

let many1CharsTillApply p endp f = many1CharsTillApply2 p p endp f

let many1CharsTill2 p1 p endp =
    many1CharsTillApply2 p1 p endp (fun str _ -> str)
        ...
        if reply.Status = Ok then
            ...
            if reply.Status <> Ok then
                ...
                if reply.Status <> Ok then
                    ...
                    if reply.Status <> Ok then
                        reply.Result <- concat3 result1 result2 result3
                    else
                        ...
                        if reply.Status <> Ok then
                            reply.Result <- concat4 result1 result2 result3 result4
                        else
                            let n =
                                2
        ...
        if reply.Status = Ok then
            ...
            if reply.Status <> Ok then
                ...
                if reply.Status = Ok then
                    ...
                    if reply.Status <> Ok then
                        result <- concat3 result1 result2 result3
                    else
                        let result4 = reply.Result
                        error0 <- error
                        stateTag0 <- stateTag
                        ...
                        if reply.Status = Ok then
                            ...
                            sb.Append(result1).Append(result2).Append(result3).Append (result4)
                            |> ignore
...
let skipped (p : Parser<unit, 'u>) : Parser<string, 'u> =
    fun stream ->
        ...
        if reply.Status = Ok then
            ...
            let nstr =
                if line0 = stream.Line then
                    str
                else
                    Text.NormalizeNewlines (str)
...
let numberLiteralE
    (opt : NumberLiteralOptions)
    (errorInCaseNoLiteralFound : ErrorMessageList)
    (stream : CharStream<'u>)
    =
    let index0 = stream.IndexToken
    ...
    if isDigit c || (c = '.' && (opt &&& allowStartingPoint) = allowStartingPoint) then
        ...
        then
            ...
            if c <> '.' then
                ...
                if c <> '0' then c <- stream.SkipAndPeek () else c <- c1
            ...
            if
                (int c ||| int ' ') = int 'e'
                && isNull error
                && (opt &&& NLO.AllowExponent) <> NLO.None
            then
...
let pfloat : Parser<float, 'u> =
    fun stream ->
        let reply =
            numberLiteralE NLO.DefaultFloat Errors.ExpectedFloatingPointNumber stream
...
let internal parseUInt64
    (c0 : char)
    (stream : CharStream<'u>)
    (status : ReplyStatus byref)
    (error : ErrorMessageList byref)
    =
            ...
            if
                (let cc = uint32 c ||| uint32 ' '
                 ...
                 if c <= '9' then
                     ...
                     cc <= uint32 'f'
                     && (nc <- cc - 0x57u
                         cc >= uint32 'a')) // 0x57u = uint32 'a' - 10u
            then
                n <- uint64 nc
                c <- stream.SkipAndPeek ()

                while (let cc = uint32 c ||| uint32 ' '
...
let internal overflowError message =
    if isNotNull message then
        messageError message // isNotNull prevents fsc from inlining the function
    else
        NoErrorMessages

let inline internal pint
    (opt : NumberLiteralOptions)
    (max : 'uint)
    (uint64_ : 'uint -> uint64)
    (uint : int -> 'uint)
    ...
    (int_ : int -> 'int)
    (errorInCaseNoLiteralFound : ErrorMessageList)
    (outOfRangeError : ErrorMessageList)
    (stream : CharStream<'u>)
    =
    ...
    if c >= '0' && c <= '9' then
        let n =
            if uint64_ max <= uint64 System.UInt32.MaxValue then
                uint_ (parseUInt32 c stream (&status) (&error))
            else
                uint__ (parseUInt64 c stream (&status) (&error))

        let isUInt32Or64 =
            uint64_ max = uint64 System.UInt32.MaxValue
            || uint64_ max = System.UInt64.MaxValue

        if
            status = Ok
            && (isUInt32Or64
                || (n <= max || (minusIsAllowed && plusMinus1 = -1 && n = max + uint 1)))
        then
...
let pint64 stream =
    pint
        ...
        int64
        int64
        Errors.ExpectedInt64
        Errors.NumberOutsideOfInt64Range
        stream

let pint32 stream =
    pint
        NumberLiteralOptions.DefaultInteger
        (uint32 System.Int32.MaxValue)
        uint64
        uint32
        uint32
        uint32
        int32
        int32
        Errors.ExpectedInt32
        Errors.NumberOutsideOfInt32Range
        stream
// fsc's optimizer seems to have problems with literals of small int types
let pint16 stream =
    pint
        NumberLiteralOptions.DefaultInteger
        ( (*uint32 System.Int16.MaxValue*) 0x7fffu)
        uint64
        uint32
        uint32
        uint32
        int16
        int16
        Errors.ExpectedInt16
        Errors.NumberOutsideOfInt16Range
        stream

let pint8 stream =
    pint
        NumberLiteralOptions.DefaultInteger
        ( (*uint32 System.SByte.MaxValue*) 0x7fu)
        uint64
        uint32
        uint32
        uint32
        ...
        sbyte
        Errors.ExpectedInt8
        Errors.NumberOutsideOfInt8Range
        stream
...
let puint64 stream =
    pint
        ...
        uint64
        uint64
        Errors.ExpectedUInt64
        Errors.NumberOutsideOfUInt64Range
        stream
...
let puint32 stream =
    pint
        ...
        uint32
        uint32
        Errors.ExpectedUInt32
        Errors.NumberOutsideOfUInt32Range
        stream
...
let puint16 stream =
    pint
        ...
        uint16
        uint16
        Errors.ExpectedUInt16
        Errors.NumberOutsideOfUInt16Range
        stream
...
let puint8 stream =
    pint
        ...
        byte
        byte
        Errors.ExpectedUInt8
        Errors.NumberOutsideOfUInt8Range
        stream
...
let notFollowedByEof : Parser<unit, 'u> =
    fun stream ->
        if not (stream.IsEndOfStream) then
            ...
            Reply (Error, Errors.UnexpectedEndOfInput)
...
let followedByString (str : string) : Parser<unit, 'u> =
    ...
    if str.Length = 1 then
        ...
        fun stream ->
            if stream.Match (chr) then
                Reply (())
            else
                Reply (Error, error)
    else
        fun stream ->
            if stream.Match (str) then
                Reply (())
            else
                Reply (Error, error)
...
let followedByStringCI str : Parser<unit, 'u> =
    ...
    if str.Length = 1 then
        ...
        fun stream ->
            if stream.MatchCaseFolded (cfChr) then
                Reply (())
            else
                Reply (Error, error)
    else
        ...
        fun stream ->
            if stream.MatchCaseFolded (cfStr) then
                Reply (())
            else
                Reply (Error, error)
...
let notFollowedByString str : Parser<unit, 'u> =
    ...
    if str.Length = 1 then
        ...
        fun stream ->
            if not (stream.Match (chr)) then
                Reply (())
            else
                Reply (Error, error)
    else
        fun stream ->
            if not (stream.Match (str)) then
                Reply (())
            else
                Reply (Error, error)
...
let notFollowedByStringCI str : Parser<unit, 'u> =
    ...
    if str.Length = 1 then
        ...
        fun stream ->
            if not (stream.MatchCaseFolded (cfChr)) then
                Reply (())
            else
                Reply (Error, error)
    else
        ...
        fun stream ->
            if not (stream.MatchCaseFolded (cfStr)) then
                Reply (())
            else
                Reply (Error, error)
