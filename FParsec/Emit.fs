...
let loadI4 (ilg : ILGenerator) (i : int32) =
    ...
    | _ ->
        ...
        if i <> int32 i1 then
            ...
            ilg.Emit (OpCodes.Ldc_I4_S, i1)

let loadI8 (ilg : ILGenerator) (i : int64) =
    let i4 = int32 i

    if i <> int64 i4 then
        ilg.Emit (OpCodes.Ldc_I8, i)
    else
        loadI4 ilg i4
        ilg.Emit (OpCodes.Conv_I8)

let loadU8 (ilg : ILGenerator) (i : uint64) =
    if i > uint64 System.UInt32.MaxValue then
...
let private createLoaderForPrimitiveConstantsImpl
    (ty : System.Type)
    (ilg : ILGenerator)
    : ('T -> unit)
    =
    ...
    if ty = typeof<int32> then
        fun x -> loadI4 ilg (box x :?> int32)
    elif ty = typeof<uint32> then
        fun x -> loadI4 ilg (int32 (box x :?> uint32))
    elif ty = typeof<int64> then
        fun x -> loadI8 ilg (box x :?> int64)
    elif ty = typeof<uint64> then
        fun x -> loadU8 ilg (box x :?> uint64)
    elif ty = typeof<int16> then
        fun x -> loadI4 ilg (int32 (box x :?> int16))
    elif ty = typeof<uint16> then
        fun x -> loadI4 ilg (int32 (box x :?> uint16))
    elif ty = typeof<char> then
        fun x -> loadI4 ilg (int32 (box x :?> char))
    elif ty = typeof<int8> then
        fun x -> loadI4 ilg (int32 (box x :?> int8))
    elif ty = typeof<uint8> then
        fun x -> loadI4 ilg (int32 (box x :?> uint8))
        ...
        fun x -> ilg.Emit (OpCodes.Ldc_R8, (box x :?> double))
    elif ty = typeof<float32> then
        fun x -> ilg.Emit (OpCodes.Ldc_R4, (box x :?> float32))
    elif ty = typeof<nativeint> then
        fun x -> loadI ilg (box x :?> nativeint)
    elif ty = typeof<unativeint> then
        fun x -> loadU ilg (box x :?> unativeint)
    else
        invalidArg "ty" "Invalid type argument."
...
let emitRangeTest pushFalseIfInRange (ilg : ILGenerator) minValue maxValue (range : Range) =
    ...
    if minValue = range.Min && range.Max = maxValue then
        ...
        if not pushFalseIfInRange then
            emitNot ()
    elif range.Max = maxValue then
        // we only have to check the left bound
        loadI4 ilg range.Min
...
let emitTwoRangeTest
    (ilg : ILGenerator)
    (loadVar : ILGenerator -> unit)
    inverse
    minValue
    maxValue
    (range1 : Range)
    ...
    if needOuterRangeCheck && (maxValue - minValue < w) then
        ...
        not needOuterRangeCheck
        || (range1.Max + 2 = range2.Min
...
let emitSetMembershipTest
    (ilg : ILGenerator)
    (loadVar : ILGenerator -> unit)
    (storeResult : ILGenerator -> unit)
    (temps : TempLocals)
    lengthCap
    densityThreshold
    minValue
    ...
    let emitBitVectorTest minValue maxValue iBegin iEnd =
        ...
        let off =
            if first > 0 && (last < w || (first < 3 * w && (last >= first + w))) then
                0
            else
                first
        ...
        let ptr =
            if length = 1 then
                NativePtr.ofNativeInt (NativePtr.toNativeInt &&stackVar)
            else
                NativePtr.ofNativeInt (UnmanagedMemoryPool.Allocate (length * sizeof<unativeint>))
            ...
            for j = 0 to length - 1 do
                ...
                while rMin < j1w do
                    ...
                    if rMin < rMax then
                        ...
                        rMin <- System.Int32.MaxValue // break
    ...
    let rec emitRegion minValue maxValue iBegin iEnd =
        Debug.Assert (
            iBegin < iEnd
            && minValue <= ranges.[iBegin].Min
            && ranges.[iEnd - 1].Max <= maxValue
        )
        ...
        | _ -> // at least 3 ranges
            if not noBitVectorTests && density lengthCap ranges iBegin iEnd >= densityThreshold then
...
let emitSwitch
    ...
    let rec emitRegion minValue maxValue iBegin iEnd =
        ...
        match iEnd - iBegin with
        | 0 -> failwith "emitSwitch.emitRegion"
        | 1 -> emitBranchIfInRange2 labels.[iBegin] defaultLabel minValue maxValue ranges.[iBegin]
        | 2 ->
            ...
            if l1 = l2 then
                ...
                if
                    rangesAreConnected
                    && ((checkLeft && checkRight) || (not checkLeft && not checkRight))
                then
                    emitBranchIfOutOfRange defaultLabel minValue maxValue (Range (r1.Min, r2.Max))
                    ...
                    (if rangesAreConnected then
                         checkRight (* not checkLeft *)
                     else
                         uint32 (r1.Max - r1.Min) >= uint32 (r2.Max - r2.Min))
                then
        ...
        | _ -> // at least 3 ranges
            let allLabelsAreIdentical =
                ...
                while i < iEnd && label.Equals (labels.[i]) do
                    i <- i + 1
            ...
            if allLabelsAreIdentical then
                ...
                emitSetMembershipTest
                    ilg
                    loadVar
                    (fun ilg -> ilg.Emit (OpCodes.Stloc, bl))
                    temps
                    (lengthCap * 8)
                    (densityThreshold / 32.)
                    minValue
